...
namespace Stockfish##x { extern int main(int argc, char* argv[]); } \
extern "C" void (*__start_##x##_init[])(void); \
extern "C" void (*__stop_##x##_init[])(void); \
 int Stockfish_##x(int argc, char* argv[]) {\
 puts("Selected: " #x); \
 unsigned count = __stop_##x##_init - __start_##x##_init; \
    for (unsigned i = 0; i < count; i++) { \
        __start_##x##_init[i](); \
    } \
  return Stockfish##x::main(argc, argv); \
}
...
int main(int argc, char *argv[]) {
    ...
    if (max_leaf < 7U || !(ecx & (1 << 19)) || !(ecx & (1 << 23))) { // no popcnt or no sse4.1
        return Stockfish_x86_64(argc, argv);
    }

    __get_cpuid_count(7, 0, &eax, &ebx, &ecx, &edx);
    if (!(ebx & (1 << 5))) { // no avx2
        return Stockfish_x86_64_sse41_popcnt(argc, argv);
    }

    if (!(ebx & (1 << 8))) { // no bmi2 (todo, detect slow Zen 2)
        return Stockfish_x86_64_avx2(argc, argv);
    }

    if (!(ebx & (1 << 16)) || !(ebx & (1 << 31)) || !(ebx & (1 << 30))) { // no avx512f/vl/bw
        __get_cpuid_count(7, 1, &eax, &ebx, &ecx, &edx);
        if (eax & (1 << 4)) {  // avxvnni
            return Stockfish_x86_64_avxvnni(argc, argv);
        } else {
            return Stockfish_x86_64_bmi2(argc, argv);
        }
    }

    if (!(ecx & 1 << 11 /* vnni512 */)) {
        return Stockfish_x86_64_avx512(argc, argv);
    }
    ...
    if (!(ebx & 1 << 21 /* ifma */) || !(ecx & 1 << 1 /* vbmi */) || !(ecx & 1 << 6 /* vbmi2 */) || !(ecx & 1 << 14 /* vpopcntdq */)
        ...
        return Stockfish_x86_64_vnni512(argc, argv);
    }

    return Stockfish_x86_64_avx512icl(argc, argv);
}