...
constexpr auto LineBB = []() {
    std::array<std::array<Bitboard, SQUARE_NB>, SQUARE_NB> bb{};
    for (Square s1 = SQ_A1; s1 <= SQ_H8; ++s1)
        for (PieceType pt : {BISHOP, ROOK})
            for (Square s2 = SQ_A1; s2 <= SQ_H8; ++s2)
                if (PseudoAttacks[pt][s1] & s2)
                    bb[s1][s2] = (sliding_attack(pt, s1, 0) & sliding_attack(pt, s2, 0)) | s1 | s2;
    return bb;
}();
    ...
    for (Square s1 = SQ_A1; s1 <= SQ_H8; ++s1)
        for (PieceType pt : {BISHOP, ROOK})
            for (Square s2 = SQ_A1; s2 <= SQ_H8; ++s2)
            {
                if (PseudoAttacks[pt][s1] & s2)
                    bb[s1][s2] =
                      sliding_attack(pt, s1, square_bb(s2)) & sliding_attack(pt, s2, square_bb(s1));
                bb[s1][s2] |= s2;
            }
    return bb;
}();

constexpr auto RayPassBB = []() {
    std::array<std::array<Bitboard, SQUARE_NB>, SQUARE_NB> bb{};
    for (Square s1 = SQ_A1; s1 <= SQ_H8; ++s1)
        for (PieceType pt : {BISHOP, ROOK})
            for (Square s2 = SQ_A1; s2 <= SQ_H8; ++s2)
                if (PseudoAttacks[pt][s1] & s2)
                    bb[s1][s2] =
                      sliding_attack(pt, s1, 0) & (sliding_attack(pt, s2, square_bb(s1)) | s2);
    return bb;
}();
