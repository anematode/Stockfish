...
Network<Arch, Transformer>::evaluate(const Position&                         pos,
    ...
    uint8_t nnz32[FTDimensions / 32];

    ASSERT_ALIGNED(transformedFeatures, alignment);

    const int  bucket = (pos.count<ALL_PIECES>() - 1) / 4;
    const auto psqt =
      featureTransformer.transform(pos, accumulatorStack, cache, transformedFeatures, bucket, nnz32);
    const auto positional = network[bucket].propagate(transformedFeatures, nnz32);
