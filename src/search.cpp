...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, const bool cutNode) {
    ...
    if (depth >= 3 && !is_decisive(beta) && !(is_valid(ttData.value) && ttData.value < probCutBeta))
    {
        ...
        MovePicker mp(pickerStorage[ss->ply], pos, ttData.move, probCutBeta - ss->staticEval, &captureHistory);
...
moves_loop:  // When in check, search starts here
    ...
    MovePicker mp(pickerStorage[ss->ply], pos, ttData.move, depth, &mainHistory, &lowPlyHistory, &captureHistory, contHist,
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    MovePicker mp(pickerStorage[ss->ply], pos, ttData.move, DEPTH_QS, &mainHistory, &lowPlyHistory, &captureHistory,
