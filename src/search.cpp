...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    bool  givesCheck, improving, priorCapture, opponentWorsening, small = false;
    ...
    if (!rootNode)
    {
        ...
        if (threads.stop.load(std::memory_order_relaxed) || pos.is_draw(ss->ply)
            || ss->ply >= MAX_PLY)
            return (ss->ply >= MAX_PLY && !ss->inCheck) ? evaluate(pos).first : value_draw(nodes);
    ...
    else if (ss->ttHit)
    {
        ...
        if (!is_valid(unadjustedStaticEval))
            std::tie(unadjustedStaticEval, small) = evaluate(pos);
    ...
    else
    {
        ...
        ttWriter.write(posKey, VALUE_NONE, ss->ttPv, BOUND_NONE, DEPTH_UNSEARCHED, Move::none(),
                       unadjustedStaticEval, tt.generation(), small);
    }
        ...
        while ((move = mp.next_move()) != Move::none())
        {
            ...
            if (value >= probCutBeta)
            {
                ...
                ttWriter.write(posKey, value_to_tt(value, ss->ply), ss->ttPv, BOUND_LOWER,
                               probCutDepth + 1, move, unadjustedStaticEval, tt.generation(), small);
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    bool  pvHit, givesCheck, capture, small = false;
    int   moveCount;
    ...
    if (pos.is_draw(ss->ply) || ss->ply >= MAX_PLY)
        return (ss->ply >= MAX_PLY && !ss->inCheck) ? evaluate(pos).first : VALUE_DRAW;
...
std::pair<Value, bool> Search::Worker::evaluate(const Position& pos) {
