...
void Search::Worker::start_searching() {
    ...
    if (Distributed::is_root())
    {
        ...
        auto bestmove = UCIEngine::move(bestMove, rootPos.is_chess960());
           ...
           && !(limits.depth && mainThread && Distributed::is_root() && rootDepth >= limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            while (true)
            {
                ...
                if (mainThread && Distributed::is_root() && multiPV == 1
            ...
            if (Distributed::is_root() && mainThread && !threads.stop
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    else
    {
        ...
        if (bestValue >= beta)
        {
            ...
            if (!ss->ttHit)
                Distributed::save(tt, threads, this, ttWriter, posKey, VALUE_NONE, false,
                                  BOUND_LOWER, DEPTH_UNSEARCHED, Move::none(), unadjustedStaticEval,
                                  tt.generation());
            return bestValue;
        }
