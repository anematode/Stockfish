...
struct TTData {
    ...
    Depth depth;
    Bound bound;
    bool  is_pv, is_smallnet;

    TTData() = delete;

    // clang-format off
    TTData(Move m, Value v, Value ev, Depth d, Bound b, bool pv, bool is_smallnet) :
        move(m),
        value(v),
        eval(ev),
        depth(d),
...
struct TTWriter {
   public:
    void write(Key k, Value v, bool pv, Bound b, Depth d, Move m, Value ev, uint8_t generation8, bool is_smallnet);
